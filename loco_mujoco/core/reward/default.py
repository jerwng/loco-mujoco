...
class VelocityWithHeadingGoalReward(Reward):
    """
    Reward for tracking velocity commands with target heading instead of yaw velocity.
    Tracks x, y velocities and heading angle.
    """

    def __init__(self, env: Any,
                 tracking_w_exp_xy: float = 5.0,
                 tracking_w_exp_heading: float = 5.0,
                 tracking_w_sum_xy: float = 2.0,
                 tracking_w_sum_heading: float = 1.0,
                 **kwargs):
        """
        ...
        Args:
            env (Any): The environment instance.
            tracking_w_exp_xy (float): Weight for exponential xy tracking reward.
            tracking_w_exp_heading (float): Weight for exponential heading tracking reward.
            tracking_w_sum_xy (float): Weight for xy tracking in total reward sum.
            tracking_w_sum_heading (float): Weight for heading tracking in total reward sum.
            **kwargs (Any): Additional keyword arguments.
        """
        super().__init__(env, **kwargs)
        ...
        self._w_exp_xy = tracking_w_exp_xy
        self._w_exp_heading = tracking_w_exp_heading
        self._w_sum_xy = tracking_w_sum_xy
        self._w_sum_heading = tracking_w_sum_heading

        # find the goal velocity observation
        assert "GoalRandomVelocityWithHeading" in env.obs_container, \
            f"GoalRandomVelocityWithHeading is the required goal for the reward for {self.__class__.__name__}"
    ...
    def __call__(self,
                 ...
                 backend: ModuleType) -> Tuple[float, Any]:
        """
        Computes tracking reward based on velocity and heading deviation.
        ...
        Args:
            ...
            data (Union[MjData, Data]): The simulation data.
            carry (Any): Additional carry.
            backend (ModuleType): Backend module used for computation (numpy or jax.numpy).

        Returns:
            Tuple[float, Any]: The reward and updated carry.
        """
        if backend == np:
            R = np_R
        else:
        ...
        goal_state = getattr(carry.observation_states, "GoalRandomVelocityWithHeading")
        ...
        # get current local velocity of root (xy only)
        lin_vel_global = backend.squeeze(data.qvel[self._vel_idx])[:3]
        lin_vel_local = root_quat.as_matrix().T @ lin_vel_global
        vel_local_xy = lin_vel_local[:2]

        # get current heading
        current_heading = root_quat.as_euler('xyz')[2]
        
        # get target from goal
        goal_vel_xy = backend.array([goal_state.goal_vel_x, goal_state.goal_vel_y])
        target_heading = goal_state.target_heading
        ...
        tracking_reward_xy = backend.exp(-self._w_exp_xy * backend.mean(backend.square(vel_local_xy - goal_vel_xy)))
        tracking_reward_heading = backend.exp(-self._w_exp_heading * backend.square(heading_error))
        total_tracking = self._w_sum_xy * tracking_reward_xy + self._w_sum_heading * tracking_reward_heading
...
class LocomotionWithHeadingReward(VelocityWithHeadingGoalReward):
    """
    Reward function extending VelocityWithHeadingGoalReward with typical additional penalties
    and regularization terms for locomotion. This reward is stateful: LocomotionRewardState
    """
    ...
    def __call__(self,
                 ...
                 backend: ModuleType) -> Tuple[float, Any]:
        """
        Based on the tracking reward with heading, this reward function adds typical penalties 
        and regularization terms for locomotion.
